play_permutation <- function(subid){
  library(dplyr)
  library(tidyr)
  library(reshape)
  library(reshape2)
...
permnum=1
permacc<- matrix(0, nrow=145, ncol=permnum)
for (i in 1:length(data_split)) {
  permacc[i,]<- svmperm(data_split[[i]])
}
write.csv(permacc,file=paste0("permres_",subid,sep=""), row.names = TRUE)
}